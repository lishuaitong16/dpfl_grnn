...
激活函数用途：
- GRNN 攻击（exp2/exp3/exp4）用 Sigmoid：需要计算"损失对梯度的梯度"（二阶导），
  ReLU 在 0 点不二阶可导，会导致攻击不稳定甚至报错。
- FL 训练（exp0/exp1）用 ReLU：收敛更稳定，精度更高。
...
class LeNet(nn.Module):
    def __init__(self, num_classes: int = 10, in_channels: int = 1, act: str = "sigmoid"):
        """
        Args:
            num_classes: 类别数，MNIST 为 10。
            in_channels: 输入通道，MNIST 灰度图为 1。
            act: "sigmoid"（GRNN 攻击必需）或 "relu"（FL 训练）。
        """
        super().__init__()
        self.act_name = act
        # 输入 1x32x32
...
def build_lenet(num_classes: int = 10, in_channels: int = 1, act: str = "sigmoid") -> LeNet:
    """工厂函数，方便实验脚本统一构造模型。"""
