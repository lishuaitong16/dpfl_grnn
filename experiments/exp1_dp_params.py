...
实验目标：固定 C=1.0、δ=1e-5，扫描每轮隐私预算 ε_round，
对比 Laplace 和 Gaussian 两种机制下精度随 ε_round 的变化，
量化差分隐私的"隐私-效用权衡"。

DP 方案：per-round delta DP（顺序合成定理）
    Laplace  噪声标准差 σ = C√2 / ε_round
    Gaussian 噪声标准差 σ = C√(2 ln(1.25/δ)) / ε_round
    总预算   ε_total = T × ε_round（T = 通信轮数）
"""
...
def parse_epsilons(s):
    """解析逗号分隔的 ε_round 列表，inf/∞/nodp 表示无 DP。"""
    vals = []
    for part in s.split(","):
        part = part.strip()
...
def save_csv(lap_acc, gau_acc, epsilons, rounds, path):
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["experiment", "mechanism", "epsilon_round", "epsilon_total", "round", "accuracy"])
        for mechanism, acc_dict in (("laplace", lap_acc), ("gaussian", gau_acc)):
            for eps in epsilons:
                ...
                for r, acc in enumerate(acc_dict[eps], 1):
                    writer.writerow(["dp_sweep", mechanism, eps_str, eps_total_str, r, f"{acc:.6f}"])
